...
if ((TOTAL_COUNT == 0)); then
    ...
    mkdir $SLICE_PATH/QUEUE-$TIMESTAMP

    CHK_FILE="$QUEUE_PATH/$QUEUE_NAME$SFX_CHUNK"
    echo move $CHK_FILE to $SLICE_PATH/QUEUE-$TIMESTAMP/$QUEUE_NAME$SFX_CHUNK
    mv  $CHK_FILE $SLICE_PATH/QUEUE-$TIMESTAMP/$QUEUE_NAME$SFX_CHUNK

    INFO_FILE="$QUEUE_PATH/$QUEUE_NAME$SFX_INFO"
    echo move $INFO_FILE to $SLICE_PATH/QUEUE-$TIMESTAMP/$QUEUE_NAME$SFX_INFO
    mv  $INFO_FILE $SLICE_PATH/QUEUE-$TIMESTAMP/$QUEUE_NAME$SFX_INFO
    
    QUEUE_FILE="$QUEUE_PATH/$QUEUE_NAME$SFX_QUEUE"
    echo move $QUEUE_FILE to $SLICE_PATH/QUEUE-$TIMESTAMP/$QUEUE_NAME$SFX_QUEUE
    mv  $QUEUE_FILE $SLICE_PATH/QUEUE-$TIMESTAMP/$QUEUE_NAME$SFX_QUEUE
