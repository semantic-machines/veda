...
module.exports = {
    ...
    */
    createRequestDelegation: function (driver, valueToSearch, valueToChoose, delegatedPosition, phase) {
        ...
        basic.execute(driver, 'sendKeys', 'veda-control[rel="v-s:delegate"] input.fulltext.tt-input', "****** PHASE#" + phase + " : ERROR = Cannot find attribute 'rel=v-s:delegate'", valueToSearch);
        ...
        driver.wait(basic.findUp(driver, '.glyphicon-chevron-up', 1, "****** PHASE#" + phase + " : ERROR = Cannot find 'glyphicon-chevron-up' button"),
            basic.FAST_OPERATION).then(function (result) {basic.clickUp(result, "****** PHASE#" + phase + " : ERROR = Cannot click on glyphicon-chevron-up")});
        driver.findElement({css:'veda-control[rel="v-s:delegate"] input.fulltext.tt-input'}).click();
