...
import { Selector, t } from 'testcafe';
  ...
  test('testDivControls', async t => {
    ...
    await t
      ...
      .wait(1000)
      .expect(Selector('div.container.sheet h3[property="rdfs:label"] span.value-holder').innerText.trim()).eql('Тест контролов')
      .typeText('veda-control#testString', 'Тестовое текстовое свойство')
      .wait(1000)
      .expect(Selector('div.container.sheet div[property="v-ui:testString"] span.value-holder').innerText.trim()).eql('Тестовое текстовое свойство')
      .typeText('veda-control[property="v-ui:testInteger"]', 'Hello Integer')
      .wait(1000)
      .expect(Selector('div.container.sheet div[property="v-ui:testInteger"]').innerText.trim()).eql('')
      .click('veda-control[property="v-ui:testInteger"]')
      .pressKey('ctrl+a delete')
      .typeText('veda-control[property="v-ui:testInteger"]', '777')
      .wait(1000)
      .expect(Selector('div.container.sheet div[property="v-ui:testInteger"] span.value-holder').innerText.trim()).eql('777')
      .typeText('veda-control[property="v-ui:testDecimal"]', 'Hello Decimal')
      .wait(1000)
      .expect(Selector('div.container.sheet div[property="v-ui:testDecimal"]').innerText.trim()).eql('')
      .click('veda-control[property="v-ui:testDecimal"]')
      .pressKey('ctrl+a delete')
      .typeText('veda-control[property="v-ui:testDecimal"]', '3,14159265')
      .wait(1000)
      .expect(Selector('div.container.sheet div[property="v-ui:testDecimal"] span.value-holder').innerText.trim()).eql('3.14 159 265')
      .click('veda-control#date')
      .pressKey('ctrl+a delete')
      .typeText('veda-control#date', '29051990')
      .click('veda-control[property="v-ui:testDecimal"]')
      .wait(1000)
      .expect(Selector('div.container.sheet div[property="v-ui:testDatetime"] span.value-holder').innerText.trim()).eql('29.05.1990')
      .click('veda-control[data-type="dateTime"]')
      .pressKey('ctrl+a delete')
      .typeText('veda-control[data-type="dateTime"]', '280519891232')
      .click('veda-control[property="v-ui:testDecimal"]')
      .wait(1000)
      .expect(Selector('div.container.sheet div[property="v-ui:testDatetime"] span.value-holder').innerText.trim()).eql('28.05.1989 12:32')
      .click(checkBox)
      .wait(1000)
      .expect(Selector('div.container.sheet div[rel="v-ui:testLink"] span#label').innerText.trim()).eql('Спецификация тестового объектного свойства')
      .click(radioButton)
      .wait(1000)
      .expect(Selector('div.container.sheet div[rel="v-ui:testLink"] span#label').innerText.trim()).eql('Спецификация тестового календарного свойства');
});
