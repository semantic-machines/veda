...
basic.getDrivers().forEach(function (drv) {
    ...
    //PHASE#2
    basic.execute(driver, 'click', '#user-info', "****** PHASE#2 > ADD+REPLY COMMENT : ERROR = Cannot click on 'user-info' button");
    driver.executeScript("document.querySelector('#add-comment').scrollIntoView(true);");
    basic.execute(driver, 'click', '#add-comment', "****** PHASE#2 > ADD+REPLY COMMENT : ERROR = Cannot click on 'add-comment' button");
    comment(driver, timeStamp);
    driver.executeScript("document.querySelector('#reply').scrollIntoView(true);");
    basic.execute(driver, 'click', '#reply', "****** PHASE#2 > ADD+REPLY COMMENT : ERROR = Cannot click on 'reexply' button");
    comment(driver, timeStamp + 1);
    driver.sleep(basic.FAST_OPERATION);
    driver.executeScript("document.querySelector('#reply').scrollIntoView(true);");
    basic.execute(driver, 'click', '#reply', "****** PHASE#2 > ADD+REPLY COMMENT : ERROR = Cannot click on 'reply' button");
    driver.executeScript("document.querySelector('em[about=\"rdfs:comment\"').scrollIntoView(true);");
    driver.wait(basic.findUp(driver, 'div[typeof="v-s:Comment"] textarea[class="form-control"]', 1, "****** PHASE#2 > ADD+REPLY COMMENT : ERROR = Cannot click on comment field"),
        basic.FAST_OPERATION).then(function(result){basic.clickUp(result);});
    driver.executeScript("document.querySelector('div[typeof=\"v-s:Comment\"] button[id=\"save\"]').scrollIntoView(true);");
    driver.wait(basic.findUp(driver, 'div[typeof="v-s:Comment"] button[id="save"]', 1, "****** PHASE#2 > ADD+REPLY COMMENT : ERROR = Cannot click on save button"),
        basic.FAST_OPERATION).then(function(result){basic.clickUp(result);});
    driver.sleep(basic.SLOW_OPERATION/2);
    driver.findElement({css:'div[id="comment-content"]'}).thenCatch(function (e) {basic.errorHandler(e, "****** PHASE#2 > ADD+REPLY COMMENT : ERROR = Cannot find new comment");});
    driver.findElement({css:'#app'}).then(function(){console.log("****** PHASE#2 > ADD+REPLY COMMENT : COMPLETE");});

    //PHASE#3
    check(driver, 3, 3, 2, 2, 3);
    ...
    driver.sleep(basic.SLOW_OPERATION/2);
    check(driver, 2, 2, 1, 1, 3);
    driver.wait(basic.findUp(driver, '#delete', 1, "****** PHASE#2 > DELETE COMMENT : ERROR = Cannot find delete buttons"), basic.FAST_OPERATION).then(function (result) {
        basic.clickUp(result);});
    driver.switchTo().alert().accept();
    driver.sleep(basic.SLOW_OPERATION/2);
    check(driver, 1, 1, 1, 1, 3);
    driver.findElement({css:'#app'}).then(function(){console.log("****** PHASE#3 > DELETE COMMENT    : COMPLETE");});
    ...
    driver.executeScript("document.querySelector('#reply').scrollIntoView(true);");
    check(driver, 1, 1, 0, 0, 4);
    driver.findElement({css:'#app'}).then(function(){console.log("****** PHASE#4 > OTHER USER        : COMPLETE");});
