...
function updateVersion(driver, task, label, valueToSearch, valueToChoose) {
    if(task == 'edit') {
        ...
        basic.execute(driver, 'click', 'button[id="edit"]', "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Cannot click on `Edit` button");
    }
    ...
    basic.execute(driver, 'clear', 'veda-control[property="rdfs:label"] div[class="input-group"] input[type="text"]',
        "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Cannot fill 'rdfs:label' field");
    basic.execute(driver, 'sendKeys', 'veda-control[property="rdfs:label"] div[class="input-group"] input[type="text"]',
        "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Cannot fill 'rdfs:label' field", label);
    ...
    basic.execute(driver, 'clear', 'div[rel="v-s:responsible"] + veda-control input[id="fulltext"]',
        "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Cannot find attribute `v-s:responsible`");
    ...
    basic.execute(driver, 'click', 'button[id="save"]', "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Cannot click on `Save` button");
}
        ...
        var a;
            driver.findElement({css: 'div[rel="v-s:previousVersion"]'}).getText().then(function (result) {
                ...
                if (a != ("МероприятиеВерсия: " + version[i])) {
                    console.trace("****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Seems wrong version, expected: " + version[i]);
                    process.exit(1);
                } else {
                    ...
                    basic.execute(driver, 'click', 'div[rel="v-s:previousVersion"] [typeof="v-s:Action v-s:Version"]',
                        "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Cannot click on 'previousVersion'");
                    driver.sleep(basic.FAST_OPERATION);
                    driver.executeScript("document.querySelector('strong[about=\"v-s:responsible\"]').scrollIntoView(true);");
                    var b = responsible[i];
                    driver.findElement({css: 'div[rel="v-s:responsible"]'}).getText().then(function (result) {
                        assert.equal("Назначение: " + b, result);
                    }).thenCatch(function (e) {basic.errorHandler(e, "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Seems wrong responsible, expected" + b);})
                    i++;
                }
            }).thenCatch(function (e) {basic.errorHandler(e, "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Invalid element");});
        });
    };
}
...
basic.getDrivers().forEach(function (drv) {
    ...
    driver.findElement({css:'#app'}).then(function(){console.log("****** PHASE#2 > CREATE ACTION1        : COMPLETE");});
    ...
    basic.execute(driver, 'click', 'div[rel="v-s:nextVersion"] [typeof="v-s:Action v-s:Version"]',
        "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Cannot click on 'nextVersion'");
    driver.executeScript("document.querySelector('strong[about=\"v-s:nextVersion\"]').scrollIntoView(true);");
    basic.execute(driver, 'click', 'div[rel="v-s:nextVersion"] [typeof="v-s:Action v-s:Version"]',
        "****** PHASE#3 > CREATE+CHECK VERSIONS : ERROR = Cannot click on 'nextVersion'");
    ...
    driver.findElement({css:'#app'}).then(function(){console.log("****** PHASE#3 > CREATE+CHECK VERSIONS : COMPLETE");});
