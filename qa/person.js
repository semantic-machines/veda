...
createPerson: function (driver, drv, lastName, firstName, middleName, date) {
    ...
    basic.execute(driver, 'sendKeys', 'div[id="object-container"] [property="rdfs:label"] + veda-control input',
        "Cannot fill 'rdfs:label' for person", lastName + " " + firstName + " " + middleName);
    basic.execute(driver, 'sendKeys', '[property="v-s:lastName"] + veda-control input', "Cannot fill 'v-s:lastName' for person", lastName);
