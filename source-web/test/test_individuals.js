...
function generate_test_document1(ticket) {
    ...
    var new_test_doc1 = {
        ...
        'v-s:test_decimal10': newDecimal(146291.02),
...
//            Backend.flush (m_fulltext_indexer, res.op_id);
    ...
    QUnit.test(
      ...
      /////////////////////// ADD TO (2)
            ...
            var new_test_doc1_set1 = {
                '@': new_test_doc1_uri,
                'rdf:type': newUri('rdfs:Resource'),
                'v-s:author': [newUri('td:test-e1')[0], newUri('td:test-e2')[0]],
                'v-s:created': now,
                'v-s:test_field': newStr('test data', 'EN')
            };
    ...
    QUnit.test("#029 test server side script: decimal, and various format [{}], {}, [[{}]]", function (assert) {
        ...
        doc["v-s:test_ArArObj"] = newInt(20000001);
    ...
    */
    ...
    QUnit.test("#037 test rights delete & restore", function (assert) {
        ...
        res = addRight(ticket_admin.id, [can_update], user1, doc1);
        ...
        right2["v-s:canUpdate"] = newBool(true);

        right2["v-s:deleted"] = newBool(true);

        res = Backend.put_individual(ticket_admin.id, right2);
        ...
        right2["v-s:canDelete"] = newBool(true);
    ...
    QUnit.test("#038 single right permissions delete & restore", function (assert) {
        ...
        right1["v-s:deleted"] = newBool(true);

        res = Backend.put_individual(ticket_admin.id, right1);
    ...
    QUnit.test("#040 Test Membership dropCount",
        function (assert) {
            ...
            var test_memb1_dropCount_uri = genUri();
            var test_memb1_dropCount = {
          ...
          Backend.wait_module(m_acl, res.op_id);
            ...
            // doc  в doc_gr с RUD
            var test_memb2_dropCount_uri = genUri();
            var test_memb2_dropCount = {
          ...
          Backend.wait_module(m_acl, res.op_id);
            ...
            var test_memb3_dropCount_uri = genUri();
            var test_memb3_dropCount = {
          ...
          Backend.wait_module(m_acl, res.op_id);
            ...
            // doc в doc_gr с RUD
            var test_memb4_dropCount_uri = genUri();
            var test_memb4_dropCount = {
          ...
          Backend.wait_module(m_acl, res.op_id);
            ...
            var test_memb5_dropCount_uri = genUri();
            var test_memb5_dropCount = {
    ...
    QUnit.test("#041 Test PermissionStatement dropCount",
        ...
        res = Backend.put_individual(ticket_admin.id, test_perm1_dropCount);
        Backend.wait_module(m_acl, res.op_id);
        ...
        res = Backend.put_individual(ticket_admin.id, test_perm3_dropCount);
        Backend.wait_module(m_acl, res.op_id);
        // #11
        check_rights_fail(assert, ticket_user.id, doc['@'], [can_read, can_update, can_delete]);
