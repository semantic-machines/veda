export default ({test, assert, Backend, Helpers, Constants, Util}) => {
  test(`#030 Check search using ranges`, async () => {
    ...
    let res = await Backend.put_individual(new_test_doc4);
    ...
    data = await Backend.query(test_group_uid);
    assert(data.result.length === 4);

    data = await Backend.query("'v-s:test_group' === '" + test_group_uid + "'");
    assert(data.result.length === 4);

    data = await Backend.query("'v-s:test_datetime0' === [2013-12-31T00:00:00, 2014-01-03T00:00:00] && 'v-s:test_group' === '" + test_group_uid + "'");
    ...
    data = await Backend.query("'v-s:test_datetime1' === [2014-04-01T00:00:00, 2014-06-03T00:00:00] && 'v-s:test_datetime0' === [2013-12-31T00:00:00, 2014-01-03T00:00:00] && 'v-s:test_group' === '" + test_group_uid + "'");
    ...
    res = await Backend.remove_individual(new_test_doc1['@']);
    assert(await Backend.wait_module(Constants.m_acl, res.op_id));
    assert(await Backend.wait_module(Constants.m_scripts, res.op_id));
    await Helpers.test_fail_read(new_test_doc1);

    res = await Backend.remove_individual(new_test_doc2['@']);
    assert(await Backend.wait_module(Constants.m_acl, res.op_id));
    assert(await Backend.wait_module(Constants.m_scripts, res.op_id));
    await Helpers.test_fail_read(new_test_doc2);

    res = await Backend.remove_individual(new_test_doc3['@']);
    assert(await Backend.wait_module(Constants.m_acl, res.op_id));
    assert(await Backend.wait_module(Constants.m_scripts, res.op_id));
    await Helpers.test_fail_read(new_test_doc3);

    res = await Backend.remove_individual(new_test_doc4['@']);
    assert(await Backend.wait_module(Constants.m_acl, res.op_id));
    assert(await Backend.wait_module(Constants.m_scripts, res.op_id));
    await Helpers.test_fail_read(new_test_doc4);
  });
};
