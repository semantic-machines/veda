export default ({it, assert, Backend, Helpers, Constants, Util}) => {
  it(`#019 Nested groups with restrictions & cycles`, async () => {
    ...
    await Helpers.addToGroup(ticket1.ticket, doc2['@'], doc3['@'], ['v-s:canRead']);
    await Helpers.addToGroup(ticket1.ticket, doc1['@'], doc2['@']);
    await Helpers.addToGroup(ticket1.ticket, doc3['@'], doc1['@'], ['v-s:canRead']);

    await Helpers.addToGroup(ticket1.ticket, doc_group1_uri, doc1['@']);
    await Helpers.addToGroup(ticket1.ticket, doc_group1_uri, doc2['@']);
    await Helpers.addToGroup(ticket1.ticket, doc_group1_uri, doc3['@']);

    await Helpers.addToGroup(ticket1.ticket, doc_group2_uri, doc_group1_uri);
    await Helpers.addToGroup(ticket1.ticket, doc_group3_uri, doc_group2_uri);

    res = await Helpers.addRight(ticket1.ticket, ticket2.user_uri, doc_group1_uri, ['v-s:canRead']);
    ...
    res = await Helpers.addRight(ticket1.ticket, ticket2.user_uri, doc_group3_uri, ['v-s:canDelete']);
    assert(await Backend.wait_module(Constants.m_acl, res[1].op_id));

    await Helpers.check_rights_success(ticket2.ticket, doc1['@'], ['v-s:canRead', 'v-s:canUpdate', 'v-s:canDelete']);

    await Helpers.check_rights_success(ticket2.ticket, doc3['@'], ['v-s:canRead', 'v-s:canUpdate', 'v-s:canDelete']);
