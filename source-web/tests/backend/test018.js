export default ({test, assert, Backend, Helpers, Constants, Util}) => {
  test(`#018 Nested groups with restrictions (2)`, async () => {
    ...
    // Stage 3: Login as user1 and setup complex group structure
    await Backend.logout();
    await Helpers.get_user1_ticket();

    await Helpers.addToGroup(doc_group1_uri, doc3['@']);
    await Helpers.addToGroup(doc_group2_uri, doc3['@']);
    await Helpers.addToGroup(doc_group1_uri, doc1['@']);
    await Helpers.addToGroup(doc_group2_uri, doc1['@']);
    await Helpers.addToGroup(doc1['@'], doc2['@']);
    await Helpers.addToGroup(doc1['@'], doc3['@'], ['v-s:canRead']);
    await Helpers.addToGroup(doc_group3_uri, doc_group1_uri);
    await Helpers.addToGroup(doc_group3_uri, doc_group2_uri);

    res = await Helpers.addRight(user2_uri, doc_group3_uri, ['v-s:canRead']);
    ...
    res = await Helpers.addRight(user2_uri, doc_group1_uri, ['v-s:canDelete']);
    assert(await Backend.wait_module(Constants.m_acl, res[1].op_id));
    ...
    await Helpers.check_rights_success(doc3['@'], ['v-s:canRead']);
    await Helpers.check_rights_success(doc3['@'], ['v-s:canUpdate']);
    await Helpers.check_rights_success(doc3['@'], ['v-s:canDelete']);
