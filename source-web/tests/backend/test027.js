export default ({test, assert, Backend, Helpers, Constants, Util}) => {
  test(`#027 Check different group subtrees (2)`, async () => {
    ...
    const res = await Helpers.addRight(user1_uri, group_B, ['v-s:canRead']);
    ...
    await Helpers.check_rights_fail(new_test_doc1['@'], ['v-s:canUpdate']);
    await Helpers.check_rights_fail(new_test_doc1['@'], ['v-s:canDelete']);
