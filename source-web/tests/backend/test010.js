export default ({test, assert, Backend, Helpers, Constants, Util}) => {
  test(`#010 Store 3 and read 3 individuals of type "v-s:NoPermissionStatement"`, async () => {
    ...
    res = await Backend.put_individual(new_test_doc2);
    assert(await Backend.wait_module(Constants.m_subject, res.op_id));
    assert(await Backend.wait_module(Constants.m_acl, res.op_id));
    assert(await Backend.wait_module(Constants.m_scripts, res.op_id));

    const server_test_doc2 = await Backend.get_individual(new_test_doc2_uri);
    ...
    res = await Backend.put_individual(new_test_doc3);
    assert(await Backend.wait_module(Constants.m_subject, res.op_id));
    assert(await Backend.wait_module(Constants.m_acl, res.op_id));
    assert(await Backend.wait_module(Constants.m_scripts, res.op_id));

    const server_test_doc3 = await Backend.get_individual(new_test_doc3_uri);
