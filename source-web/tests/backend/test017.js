export default ({test, assert, Backend, Helpers, Constants, Util}) => {
  test(`#017 Nested groups with restrictions (1)`, async () => {
    ...
    await Helpers.check_rights_fail(ticket2.ticket, doc3['@'], ['v-s:canUpdate']);

    await Helpers.check_rights_fail(ticket2.ticket, doc3['@'], ['v-s:canDelete']);

    await Backend.remove_individual(ticket1.ticket, doc1['@']);
    await Helpers.test_fail_read(assert, ticket1, doc1);
    ...
    await Helpers.test_fail_read(assert, ticket1, doc3);
  });
};
