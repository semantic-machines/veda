...
$.fn.veda_numeration = function( options ) {
  const opts = $.extend( {}, $.fn.veda_numeration.defaults, options );
  const that = this;
  const control = $(opts.template);
  const spec = opts.spec;
  const placeholder = this.attr('placeholder') || (spec && spec.hasValue('v-ui:placeholder') ? spec['v-ui:placeholder'].map(Util.formatValue).join(' ') : '');
  const property_uri = opts.property_uri;
  const individual = opts.individual;
  const input = $('.form-control', control);
  const button = $('.get-numeration-value', control);
  ...
  input.on('change focusout', function (e) {
    const value = opts.parser(e.delegateTarget.value);
    change(value);
  });
  ...
  button.on('click', function () {
    const prop = new veda.IndividualModel(property_uri);
    if (prop['v-s:hasNumerationMapper']) {
      for (const mapkey in prop['v-s:hasNumerationMapper']) {
        if (Object.hasOwnProperty.call(prop['v-s:hasNumerationMapper'], mapkey)) {
          const map = prop['v-s:hasNumerationMapper'][mapkey];
          if (map['v-s:numerationClass'][0].id == individual['rdf:type'][0].id) {
            const numertor = map['v-s:hasNumerationRule'][0];
            const scope = eval(numertor['v-s:numerationScope'][0].toString())(null, individual);
            const nextValue = eval(numertor['v-s:numerationGetNextValue'][0].toString())(null, scope);
