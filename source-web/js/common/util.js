...
Util.queryInbox = function (taskBlank, sort, withDeleted) {
  const re = /[^a-zA-Z0-9]/g;
  const task_from = taskBlank.get('v-wf:from');
  const task_to = taskBlank.get('v-wf:to');
  const task_label = taskBlank.get('rdfs:label');
  let task_created = taskBlank.get('v-s:created');
  let task_given = taskBlank.get('v-wf:dateGiven');
  const task_completed = taskBlank.get('v-wf:isCompleted');
  let document_type;
  let document_label;
  let decision_created;

  if ( taskBlank.hasValue('v-wf:onDocument') ) {
    const document = taskBlank.get('v-wf:onDocument')[0];
    document_type = document.get('rdf:type')[0];
    document_label = document.get('rdfs:label')[0];
  }
  if ( taskBlank.hasValue('v-wf:takenDecision') ) {
    const decision = taskBlank.get('v-wf:takenDecision')[0];
    decision_created = decision.get('v-s:created');
  }

  let from = 'veda_tt.`v-wf:DecisionForm` AS t0';
  if (document_type) {
  ...
  if (!withDeleted) {
    const where_deleted = !withDeleted ? 'NOT (t0.`v_s_deleted_int` = [1])' : false;
    where_arr.push('(' + where_deleted + ')');
  }
  if (task_completed.length) {
    const bool = task_completed[0];
    const where_task_completed = 't0.`v_wf_isCompleted_int` = [' + (bool ? 1 : 0) + ']';
    where_arr.push('(' + where_task_completed + ')');
  }
  if (task_from.length) {
    const where_task_from = task_from.map(function (val) {
      return 'has(t0.`v_wf_from_str`, \'' + val + '\')';
    }).join(' OR ');
    where_arr.push('(' + where_task_from + ')');
  }
  if (task_to.length) {
    const where_task_to = task_to.map(function (val) {
      return 'has(t0.`v_wf_to_str`, \'' + val + '\')';
    }).join(' OR ');
    where_arr.push('(' + where_task_to + ')');
  }
  if (task_label.length) {
    const where_task_label = task_label
      .filter(Boolean)
      .map( function (value) {
        const q = value;
        const lines = q.trim().split('\n');
  ...
  if (task_created.length) {
    task_created = task_created.sort(function (a, b) {
      return a - b;
    });
    let start = new Date(task_created[0]);
    let end = new Date(task_created[task_created.length - 1]);
    start.setHours(0, 0, 0, 0);
    end.setHours(23, 59, 59, 999);
    start = Math.floor(start.valueOf() / 1000);
    end = Math.floor(end.valueOf() / 1000);
    const where_task_created = 't0.`v_s_created_date`[1] >= toDateTime(' + start + ') AND t0.`v_s_created_date`[1] <= toDateTime(' + end + ')';
    where_arr.push('(' + where_task_created + ')');
  }
  if (task_given.length) {
    task_given = task_given.sort(function (a, b) {
      return a - b;
    });
    let start = new Date(task_given[0]);
    let end = new Date(task_given[task_given.length - 1]);
    start.setHours(0, 0, 0, 0);
    end.setHours(23, 59, 59, 999);
    start = Math.floor(start.valueOf() / 1000);
    end = Math.floor(end.valueOf() / 1000);
    const where_task_given = 't0.`v_wf_taskGiven_date`[1] >= toDateTime(' + start + ') AND t0.`v_wf_taskGiven_date`[1] <= toDateTime(' + end + ')';
    where_arr.push('(' + where_task_given + ')');
  }
  if (document_type) {
    const where_document_type = 'has(t1.str, \'' + document_type.id + '\')';
    where_arr.push('(' + where_document_type + ')');
  }
  if (document_label) {
    const where_document_label = [document_label]
      .filter(Boolean)
      .map( function (value) {
        const q = value;
        const lines = q.trim().split('\n');
  ...
  if (decision_created && decision_created.length) {
    decision_created = decision_created.sort(function (a, b) {
      return a - b;
    });
    let start = new Date(decision_created[0]);
    let end = new Date(decision_created[decision_created.length - 1]);
    start.setHours(0, 0, 0, 0);
    end.setHours(23, 59, 59, 999);
    start = Math.floor(start.valueOf() / 1000);
    end = Math.floor(end.valueOf() / 1000);
    const where_decision_created = 't3.`v_s_created_date` >= toDateTime(' + start + ') AND t3.`v_s_created_date` <= toDateTime(' + end + ')';
    where_arr.push('(' + where_decision_created + ')');
  }
  const where = where_arr.join(' AND ');

  let query = 'SELECT id FROM ' + from + ' WHERE ' + where;
  const group = groupBy(sort);
