            ...
            if (type['v-s:hasNumeration']) {
                ...
                if (individual[key][0].data > 0) {
                    ...
                    if (typeof oldstate === "undefined"
                            || !(oldstate[key] && oldstate[key][0].data > 0)
                            || (oldstate['v-s:isDraftOf'] && oldstate['v-s:isDraftOf'][0].data.length > 0)) {
                        if (commitValue(ticket, scope,
                                parseInt(individual[key][0].data), _event_id)) {
                            return {
                                'sucess' : true,
                        ...
                        if (scopeId == oldScopeId
                                ...
                                && (!individual['v-s:deleted']
                                        || !oldstate['v-s:deleted'] || (!(individual['v-s:deleted'][0].data === true && oldstate['v-s:deleted'][0].data === false)))) {
                            ...
                            if ((!oldstate['v-s:deleted'] || oldstate['v-s:deleted'][0].data === false)
                                ...
                                // document deleted
                                revokeValue(ticket, scope,
                                        parseInt(individual[key][0].data),
                                        _event_id);
                                return {
                                    'sucess' : true,
                            ...
                            if (commitValue(ticket, scope,
                                    parseInt(individual[key][0].data),
                                    _event_id)) {
                                var oldScope = get_individual(ticket,
                                        oldScopeId);
                                revokeValue(ticket, oldScope,
                                        parseInt(oldstate[key][0].data),
                                        _event_id);
                                return {
                                    'sucess' : true,
                                    'result' : 'VALUE IS COMMITED'
...
function getScope(ticket, individual, numeration) {
    try {
        return eval(numeration['v-s:numerationScope'][0].data)(ticket,
                individual);
    } catch (e) {
        print(e.stack);
    }
        ...
        if (scope['v-s:numerationCommitedInterval']) {
            ...
            for ( var i in scope['v-s:numerationCommitedInterval']) {
                ...
                if ((interval['v-s:numerationCommitedIntervalBegin'][0].data <= value)
                        && (value <= interval['v-s:numerationCommitedIntervalEnd'][0].data)) {
            ...
            if (prevInterval != null && nextInterval != null) {
                ...
                // prev = prev+next
                prevInterval['rdfs:label'][0].data = prevInterval['v-s:numerationCommitedIntervalBegin'][0].data
                        + ' - '
                        + nextInterval['v-s:numerationCommitedIntervalEnd'][0].data;
                ...
                nextInterval['rdfs:label'][0].data = value
                        + ' - '
                        + nextInterval['v-s:numerationCommitedIntervalEnd'][0].data;
...
function revokeValue(ticket, scope, value, _event_id) {
    try {
        ...
        for ( var i in scope['v-s:numerationCommitedInterval']) {
                    ...
                    put_individual(
                            ticket,
                            {
                                ...
                                'rdfs:label' : [ {
                                    data : (value + 1)
                                            + ' - '
                                            + interval['v-s:numerationCommitedIntervalEnd'][0].data,
                                    type : _String
                                } ],
                                'rdf:type' : [ {
                                    data : 'v-s:NumerationCommitedIntervalClass',
            ...
            } else if ((interval['v-s:numerationCommitedIntervalBegin'][0].data < value)
                    && (value < interval['v-s:numerationCommitedIntervalEnd'][0].data)) {
