 ...
 *    + v-s:numerationNextValue - script, returns next uncommitted value from rule scope. Returns fist value if scope is empty.
 *
 * 2. How it works
 *
 *    2.1 You must specify `v-s:hasNumerationMapper` values for your individual property
 *
 *    2.2 Add one or may `v-s:NumerationMapper` with values:
 *        + v-s:numerationClass - link to `owl:Class` - class that will be numerated against rule
 ...
 *
 *    2.5 add `rdfs:subClassOf v-s:Enumerated ;` to your class description to trigger numeration event on server side.
 *
...
function numerate(ticket, individual, super_classes, oldstate, _event_id) {
  ...
  for (var key in individual) {
    ...
    if (property && property['v-s:hasNumerationMapper']) {
      ...
      //print ('4'+key);
      var numeration = get_individual(ticket, rule);
...
function commitValue(ticket, scope, value, _event_id) {
  ...
  if (scope['v-s:numerationCommitedInterval']) {
    ...
    if (prevInterval != null && nextInterval != null) {
      ...
      scope['v-s:numerationCommitedInterval'].push({data:interval['@'], type: _Uri});
      put_individual(ticket, scope, _event_id);
    }
  } else {
    ...
    scope['v-s:numerationCommitedInterval'] = [{data:interval['@'], type: _Uri}];
    put_individual(ticket, scope, _event_id);
  }
  return true;
  } catch(e) {
