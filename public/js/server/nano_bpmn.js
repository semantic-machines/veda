...
veda.Module(function (veda) { "use strict";
  ...
  veda.bpmn = veda.bpmn || {};
    ...
    veda.bpmn.is_all_executors_taken_decision = function (value) {
          ...
          var count_agreed = 0;
          for (var i = 0; i < f_work_order.length; i++) {
            var work_order = get_individual(ticket, veda.Util.getUri(f_work_orders[i]));
	    var decision_form = get_individual(ticket, veda.Uti.getUri(work_order['bpmn:hasDecisionForm']));
	    var decision = veda.Util.getUri(decision_form['bpmn:hasDecisionForm']);
          ...
          if (count_agreed == f_work_order.length)
              return true;
          else
              return false;
