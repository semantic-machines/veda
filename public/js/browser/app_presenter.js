...
Veda(function AppPresenter(veda) { "use strict";
	...
	function login () {
		...
		$("#submit", container).on("click", function (e) {
			e.preventDefault();
			var authResult = veda.authenticate( $("#login", container).val(), Sha256.hash( $("#password", container).val() ) );
			if (!authResult) return login();
			return logged(authResult);
		});

		container.fadeIn(250);
	}

	function logged (authResult) {
