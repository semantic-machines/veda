...
Пользователь посредством веб интерфейса производит создание или модификацию сущностей, которые отправляются через HTTP API на серверную часть. На серверной стороне, измененные сущности после проверки прав доступа и сохранения в мастер базу, складываются в очередь. При этом в очередь сохраняются два состояния, сущность до изменений и ее новое состояние.
