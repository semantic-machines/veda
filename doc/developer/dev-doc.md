...
| 'rdf:type'       | косвенно можно указать в какой базе искать: сейчас поток индексация делится на несколько частей: basic, system, az и deleted |
...
| [ЗНАЧЕНИЕ1, ЗНАЧЕНИЕ2] | поиск по диапазонам <br />пример:<br/>( ( 'rdf:type'==='mnd-s:Idea' ) && ( 'v-s:created'==[2017-05-31T21:00:00.000Z,2017-06-30T20:59:59.999Z] ) ) |
...
| запрос                                                       | что ищем                                                     |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| '*' == 'лесовик'                                             | сущности содержащие слова [лесовик] по любому полю любых сущностей |
| '\*' == 'лес*'                                               | сущности содержащие  слова начинающихся на [лес] по любому полю любых сущностей |
| '\*' == 'лес\*' && '\*' == 'вод*'                            | сущности содержащие слова начинающиеся на [лес] И начинающиеся на [вод] по любым полям любых сущностей |
| '\*' == 'лес\*' \|\| '\*' == 'вод\*'                         | сущности содержащие слова начинающиеся на [лес] ИЛИ начинающихся на [вод] по любым полям любых сущностей |
| '\*' == 'лес\*' && ('\*' == 'вод\*' \|\| '\*' == 'мотор\*')  | сущности содержащие слова начинающиеся на [лес] И начинающиеся на [вод] или [мотор] по любым полям любых сущностей |
...
| 'rdf:type'==='mnd-s:Idea'  &&  'v-s:created'==[2017-05-31T21:00:00.000Z,2017-06-30T20:59:59.999Z] | сущности типа mnd-s:Idea с датой создания в диапазоне от 2017-05-31T21:00:00.000Z до 2017-06-30T20:59:59.999Z |
...
Назначение полей, генерация дополнительных данных, обзор алгоритма авторизации

**Включение реальных людей в группы должностей (на сервере).**

- При сохранении индивида класса Appointment запускается обработчик.
- Создается индивид класса принадлежность (Membership).
- Значением предиката v-s:resource устанавливается значение предиката v-s:employee из назначения.
- Значением предиката v-s:memberOf устанавливается значение предиката v-s:occupation из назначения.
- Выполняется команда put_individual для принадлежности.
- Выполнение обработчика завершается.

**Назначение прав автору нового индивида.**

*На стороне клиента:*

- При создании нового индивида, клиентское приложение дополняет индивид предикатами v-s:author, v-s:created.
- Значением предиката v-s:author устанавливается текущее (выбранное) назначение пользователя.
- Значением предиката v-s:created устанавливается текущий момент времени.
- В момент выполнения команды сохранения индивида, клиент выполняет команду put_individual.

*На стороне сервера (server js script):*

- При выполнении команды (put_individual) запускается обработчик.
- Считывается предыдущее состояние записываемого индивида.
- Если предыдущего состояния не существует, то индивид считается новым, и для него необходимо установить права доступа. В противном случае сравниваются значения предикатов v-s:author старого и нового состояния индивида.
...
- Информация об авторе индивида извлекается из предиката v-s:author.
- Выполняется проверка соответствия пользователя от имени которого идет операция записи и назначения, указанном в предикате v-s:author.
...
**Авторизация доступа к индивиду.**
