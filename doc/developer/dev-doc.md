...
Приложение для своего функционирования может требовать наличие других модулей, это задается в в разделе зависимости, в обьекте который описывает сам модуль.
...
Пользователь посредством веб интерфейса производит создание или модификацию обьектов, которые отправляются через HTTP API на серверную часть. На серверной стороне, измененные обьекты после проверки прав доступа и сохранения в мастер базу, складываются в очередь. При этом в очередь сохраняются два состояния, обьект до изменений и новое состояние.



#### 3.3. HTTP API и webscoket API
...
- Получение 
- Сохранение обьекта или группы обьектов.
- Чтение обьектов или группы обьектов по их ID.
- Удаление обьекта.
- Добавление, обновление, удаление заданного поля в обьекте.
- Получение разрешений для текущего пользователя и обьекта.
- Загрузка и скачивание файлов.

* поддерживает анонимный доступ c ограниченными правами.

**websocket API служит для оперативного получения изменений в обьектах и состоит из следующих команд:**

- получить текущий набор подписок 

- подписка на изменения одного или несколько обьектов

- отменить подписку для одного или несколько обьектов
