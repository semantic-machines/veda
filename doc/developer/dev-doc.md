   * [Платформа VEDA. Руководство по разработке приложений](#платформа-veda-руководство-по-разработке-приложений)
      ...
      * [3. Устройство платформы](#3-устройство-платформы)
         ...
         * [3.2. Архитектура платформы](#32-архитектура-платформы)
            ...
            * [3.2.2. Жизненный цикл обработки информации](#322-жизненный-цикл-обработки-информации)
         * [3.3. API для клиентского слоя](#33-api-для-клиентского-слоя)
         * [3.4. Механизмы платформы](#34-механизмы-платформы)
            ...
            * [3.4.2. Подсистема безопасности](#342-подсистема-безопасности)
               * [3.4.2.1. Описание и принцип работы](#3421-описание-и-принцип-работы)
               * [3.4.2.2. Создание схемы авторизации](#3422-создание-схемы-авторизации)
               * [3.4.2.3. Алгоритмы авторизации](#3423-алгоритмы-авторизации)
...
Пользователь в начале работы с системой должен получить сессионный тикет, для этого он отправляет на сервер данные с помощью которых происходит его идентификация. Для получения тикета необходимо передать login и хеш (SHA-256) взятый от пароля. Полученный тикет имеет ограниченное время жизни. Если делать запросы после истечения времени жизни тикета, то будет возвращаться соответствующий код ошибки.
...
##### 3.4.2.2. Создание схемы авторизации
...
1. Ограниченные прав на основе признака.

Данный вид прав позволяет ограничить доступ на отдельный ресурс.



2. Ограниченные прав на основе групп.

Данный вид прав позволяет ограничить доступ для отдельных групп пользователей. Такое ограничение позволяет наложить запрет на права полученные пользователем косвенным образом. Может понадобится если в системе есть пользователи из разных организаций.
...
```
cfg:Administrator1 a v-s:Person;  
    v-s:birthday "2014-03-04T21:00:00.000Z"^^xsd:dateTime;
    v-s:firstName "Администратор"@ru, "Administrator"@en.
```

```
INSERT INTO `rdf:type` (doc_id, doc_type, created, value, lang, deleted) VALUES (cfg:Administrator1, v-s:Person, 2019-03-26T11:33:25Z, v-s:Person, NONE 0);

INSERT INTO `v-s:firstName` (doc_id, doc_type, created, value, lang, deleted) VALUES (cfg:Administrator1, v-s:Person, 2019-03-26T11:33:25Z, Администратор, RU 0);

INSERT INTO `v-s:firstName` (doc_id, doc_type, created, value, lang, deleted) VALUES (cfg:Administrator1, v-s:Person, 2019-03-26T11:33:25Z, Administrator, EN 0);

INSERT INTO `v-s:birthday` (doc_id, doc_type, created, value, lang, deleted) VALUES (cfg:Administrator1, v-s:Person, 2019-03-26T11:33:25Z, 2014-03-04T21:00:00Z, NONE 0);
```
