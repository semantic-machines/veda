...
//	int checktime = 30;
    ...
    string[] uris = context.get_individuals_ids_via_query(&sticket, "'rdfs:isDefinedBy.isExists' == true", null, null, 100000, 100000).result;
    ...
    if (need_remove_ontology)
    {
        ...
        uris = context.get_individuals_ids_via_query(&sticket, "'rdf:type' == 'v-s:TTLFile'", null, null, 1000, 1000).result;
        foreach (uri; uris)
        {
            log.tracec("WARN: [%s] WILL BE REMOVED", uri);
