...
func prepareElement(v interface{}) interface{} {
	switch v.(type) {
	...
	case cbor.CBORTag:
		...
		if tag == TAG_TEXT_RU {
			...
			return []interface{}{map[string]interface{}{
				"data": time.Unix(wrappedVal.(int64), 0).Format("2006-01-02T15:04:05Z"),
				"type": dataTypeToString(Decimal),
			}}
		}
